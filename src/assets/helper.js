...
export function isSwappable(board, index) {
  ...
  if (index[0] - 1 >= 0 && board[index[0] - 1][index[1]] === size**2) return true;

  if (index[0] + 1 < size && board[index[0] + 1][index[1]] === size**2) return true;

  if (index[1] - 1 >= 0 && board[index[0]][index[1] - 1] === size**2) return true;

  if (index[1] + 1 < size && board[index[0]][index[1] + 1] === size**2) return true;
...
export function getSwappableIndexes(board, index) {
  ...
  if (index[0] - 1 >= 0 && board[index[0] - 1][index[1]] === size**2)
    return [index, [index[0] - 1, index[1]]];

  if (index[0] + 1 < size && board[index[0] + 1][index[1]] === size**2)
    return [index, [index[0] + 1, index[1]]];

  if (index[1] - 1 >= 0 && board[index[0]][index[1] - 1] === size**2)
    return [index, [index[0], index[1] - 1]];

  if (index[1] + 1 < size && board[index[0]][index[1] + 1] === size**2)
    return [index, [index[0], index[1] + 1]];
}
...
export function getSpaceIndex(board) {
  ...
  for (let i = 0; i < size; i++) {
    for (let j = 0; j < size; j++) {
      if (board[i][j] === size**2) return [i, j];
    }
  }
}
